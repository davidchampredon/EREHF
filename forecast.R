...
### Same as 'RESuDe.forecast' but
### less parameters have been fitted:
RESuDe.forecast.light <- function(prm, # <-- sampled parameter values after the fit
								  fcast.horizon,
								  kappa,
								  alpha,
								  GI_span, 
								  ...
								  CI2 = 95,
								  seed=123){
	message("\nStarting RESuDe forecast simulations ...\n\n")
	library(snowfall)
	sfInit(parallel = TRUE, 
		   cpu = parallel::detectCores())
	# Future time:
	...
	simul.wrap <- function(i, 
		...
		pop_size_i <- prm$pop_size[i]
		R0_i       <- prm$R0[i]
		
		tmp <-  RESuDe.simulate(# fitted parameters:
								pop_size = pop_size_i,
								R0       = R0_i,
								# non-fitted paramerters:
								I.init   = I.init,
								alpha    = alpha,
								kappa    = kappa,
								GI_span  = GI_span,
	...
	### Parallel execution:
	sfExportAll()
	idx.apply <- (max(1,nsamples-nmax)):nsamples
	...
	# Matrix storing all forecasts:
	sf <- matrix(NA, nrow = length(res), ncol = last.obs+fcast.horizon)
	for(i in 1:length(res)) sf[i,] <-res[[i]]$I
